...
- **No project save/load** — `CanvasState.isDirty` exists but nothing persists canvas state to the backend (only image uploads are persisted via Supabase Storage).
