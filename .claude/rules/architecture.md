...
1. **CanvasWrapperService** (`features/editor/canvas/`) — dynamically loads Fabric.js via `await import('fabric')` in `init()`, owns the `Canvas` instance. Exposes `isReady` signal for loading state. Bridges imperative canvas ops to Angular reactivity via signals (`selectedObjectIds`, `selectedObjectType`, `isReady`) and RxJS Subjects (`onObjectModified$`, `onTextChanged$`, `onObjectsReordered$`, etc.). Every element gets `crypto.randomUUID()` as its `id`. Provides layer ordering (bringForward/sendBackward), visibility toggles, lock/unlock, duplicate, group/ungroup, and `getElementProperties()`. **Viewport management**: `fitToScreen()` resizes the canvas DOM element to match its container (preventing overflow into sidebars), then uses viewport transform to center and scale the project content. Tracks `projectWidth`/`projectHeight` for logical dimensions separate from viewport size. `withProjectDimensions()` temporarily restores project dimensions for accurate serialization (save/export). `toFabricJSON()` clamps AI element positions to a 60px safe zone and enforces minimum text width of 200px.
